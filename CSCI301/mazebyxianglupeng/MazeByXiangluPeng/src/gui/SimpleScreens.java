...
public class SimpleScreens {
     ...
     * @param panel holds the graphics for the off-screen image
     */
	void redrawFinish(MazePanel panel, boolean result, int pathLen, float energy) {
		Graphics g = panel.getBufferGraphics() ;
        if (null == g) {
            System.out.println(errorMsg) ;
        }
        	...
        	if (result) {
        		redrawFinishWinning(g,pathLen,energy);
        	}
            if (!result){
            	redrawFinishLosing(g,pathLen,energy);
            }
        }
	}
	/**
	 ...
	 */
	private void redrawFinishWinning(Graphics gc,int pathLen, float energy) {
		...
		// write energy consumption and pathlength
		updateFontAndColor(gc, smallBannerFont, goldWM);
		centerString(gc, "Energy consumed:" + energy, 200);
		updateFontAndColor(gc, smallBannerFont, goldWM);
		centerString(gc, "Path used:" + pathLen, 220);
		// write the instructions
		gc.setColor(blackWM);
		centerString(gc, "Hit any key to restart", 300);
	}
	
	void redrawFinishLosing(Graphics gc, int pathLen, float energy) {
